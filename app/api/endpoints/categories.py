...
@limiter.limit("20/minute")
async def create_category(request: Request, new_cat: CategoryCreate, current_user: User = Depends(get_current_admin_user), db:Session = Depends(get_db)):
...
@limiter.limit("20/minute")
async def update_category(request: Request, cat_id: int, new_cat: CategoryUpdate, current_user: User = Depends(get_current_admin_user), db:Session = Depends(get_db)):
...
@limiter.limit("20/minute")
async def delete_category(request: Request, cat_id: int, current_user: User = Depends(get_current_admin_user) , db:Session = Depends(get_db)):
