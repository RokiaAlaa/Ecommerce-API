...
async def login(
    ...
    user = (
        db.query(User)
        .filter(
            (form_data.username == User.username) | (form_data.username == User.email)
        )
        .first()
    )
...
async def update_current_user(
    ...
    if user_update.email:
        existing = (
            db.query(User)
            .filter(user_update.email == User.email, current_user.id != User.id)
            .first()
        )
    ...
    if user_update.username:
        existing = (
            db.query(User)
            .filter(user_update.username == User.username, current_user.id != User.id)
            .first()
        )
